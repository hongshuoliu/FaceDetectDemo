...
import com.opencv.detect.callback.SkinFinder;
 ...
 * @date 2020-07-22
 */
public class BeautySkinPresenter {

    //
    private float sigma = 30.0f;

    BeautySkinFilter skinFilter;

    public BeautySkinPresenter() {
        skinFilter = new BeautySkinFilter();
    }

    /**
     * 使用积分图实现均值模糊
     */
    public void imageFilter(String imagePath, int actOption, ImageView imageView) {
    ...
    public void beautySkin(String imagePath, ImageView imageView, boolean isNative) {
        Mat src = Imgcodecs.imread(imagePath);
        if (src.empty()) {
            return;
        }
        ...
        sqsum.get(0, 0, data2);
        if (isNative) {
            skinFilter.beautySkinFilter(src.getNativeObjAddr(), dst.getNativeObjAddr(), sigma, false);
        } else {
            generateMask(src, mask);
            FastEPFilter(src, data1, data2, dst);
    ...
    private void generateMask(Mat src, Mat mask) {
        ...
        Mat ycrcb = new Mat();
        SkinFinder skinFinder = new DefaultSkinFinder();
